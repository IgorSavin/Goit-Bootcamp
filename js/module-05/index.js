...
//  */
//  getUsersByGender = (arr, gender) => arr.filter(item =>item.gender === gender).map(item => item.name);
//
...
//  */
// const getUserByEmail = (arr, email) => arr.filter(item => item.email === email);
//
 ...
 */
// const getTotalBalance = arr => arr.map(item=>item.balance).reduce((acc,item) => acc + item);
//
 ...
 */
// const getUsersByFriend = (arr, name) => arr.filter(item => item.friends.includes(name));
//
